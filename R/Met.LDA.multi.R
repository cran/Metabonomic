...
samp <- c(sample(p[1]:p[length(p)],v3), sample(q[1]:q[length(q)],v5),
 sample(r[1]:r[length(r)],v5b))  #Random Selection
  }
...
{
  par(bg=color)
plot(scores.total[,1],scores.total[,2],ylim=range(scores.total[,1]),
...
text(scores.total[,1],scores.total[,2],labels=abbreviate(etiqueta),cex=size$cex,col=color2)
}
...
{
  par(bg=color)
plot(representacion[,1],representacion[,2],ylim=range(representacion[,1]),
xlab=xlab,ylab=ylab,main=main,sub=subt,type="n",cex.axis=size$cex.axis,
cex.lab=size$cex.lab,cex.main=size$cex.main,cex.sub=size$cex.sub,
col.axis=col$axis,col.lab=col$lab,col.main=col$main,col.sub=col$sub)
text(scores.total[,1],scores.total[,2],labels=abbreviate(etiqueta$a),cex=size$cex,col=color2)
text(nuevos.datos[,1],nuevos.datos[,2],labels=abbreviate(etiqueta$b),cex=size$cex,col=color3)
}
img <- tkrplot(tt,plotFunction(color,color2,etiqueta,xlab,ylab,main,subt),hscale=1.5,vscale=1.5)
