...
function()
{
  ...
  memory<<-memory+1
  Met.spectrum2<-function(xCoords){
tit<-xCoords$info
xCoords<-xCoords$datos
require(tcltk)
require(tkrplot)
color="white"
color2="black"
...
tkgrid(tklabel(tt,text="Delete any spectrum?"))
tkbind(tl,scr)
tkgrid(tl,scr)
...
spectrum <-as.vector(datos$info[,1]) 
...
{
   tkinsert(tl,"end",spectrum[i])
}
tkselection.set(tl,0)  # 
onOK<-function()
{
a<-Index[,1]+1
datos<<-list(datos=datos$datos[,-a],info=datos$info[-Index[,1],])
tkdestroy(tt)
}
...
tkfocus(tt)
  ...
  Met.spectrum2(datos) #plot spectrum
